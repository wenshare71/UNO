...
> 别拿旧编号来对现在这份。
>
> **进度**:§6.0 ✅ 已过 · §6.3 floor ✅ 已跑完 60 张 · §6.1 判据 ⏳ 待跑 · §6.2 主批 ⏳ 待投。
...
1. `qwen/diag_kv.py` 打出 **FAIL**(§6.1)—— 缓存有逻辑错,后面 720 张全废。
   注意它自己会打 PASS / PASS_ROUNDING / FAIL,**只有 FAIL 才停**。
...
### 6.1 ⏳ 缓存判据 `diag_kv.py`(1 卡,约 4 分钟)—— 这是唯一的门禁

**背景**:「隔离+缓存」和「隔离+每步重算」的图有 mean 2.3–5.9 的像素差,一直没结掉。
之前两个实验都判不了它(判据错,已删)。作者读完代码后确认:**两条路 bitwise 相等
在物理上就不可能** —— read 模式只把噪声段喂进 forward(`pipeline_iso.py:246`),
img 流每个 GEMM 的 M 维从 ~12.7k 掉到 ~4.1k,cuBLAS 按形状换 kernel,累加顺序就变。
所以"像素差是不是 0"根本不是判据。

**唯一该问的是**:缓存里存的那份 ref K/V,是不是第 k 步重算会得到的那份。
隔离注意力的地基要求它必须是(ref 只自注意 + ref 段固定 t=0 调制 ⇒ ref K/V 与 t、
与噪声内容解耦)。P1 T3 在 CPU fp32 上证死过(实测 0),没验的是真权重 bf16 + GPU kernel。
...
默认跑任务表第 0 条(`M6_S1_000_s0`,2-ref)。**脚本自己打结论,你不用判**:

| 它打的 | 你做什么 |
|---|---|
| `✅ PASS` | 60 层 K/V 全逐位相同 ⇒ **直接投 §6.2 主批,不用等作者** |
| `✅ PASS(舍入级)` | 有层不等但相对差 < 1e-2,即 bf16 最后几位 ⇒ **一样直接投主批**,数记进报告 |
| `❌ FAIL` | 相对差与张量同阶 = 逻辑错 ⇒ **🔴 停**,把输出 + `diag_kv.json` 发给作者 |

顺带白拿一个数:同一时刻 write 路与 read 路在噪声 token 速度上的相对 L2,
那是"每步扰动"的真实大小。跟最终像素差 ~1e-2(2–6/255)一比就知道 40 步放大了多少倍。
**记进报告**(它不是门禁,多大都不停)。
