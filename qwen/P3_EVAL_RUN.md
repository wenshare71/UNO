...
> §6.1 判据 第一轮已跑,把问题收窄到「ref K/V 随文本段变」这一件事上,**第二轮待跑** ·
> §6.2 主批 ⏳ 待投。
...
### 6.1 ⏳ 缓存判据 `diag_kv.py` 第二轮(1 卡,约 6 分钟)—— 唯一的门禁
...
| 它打的 | 含义 | 你做什么 |
|---|---|---|
| `✅ PASS` | 换掉 txt 内容 ref K/V 逐位不变 ⇒ 良性的长度效应 | **直接投 §6.2 主批,不用等作者** |
| `❌ FAIL_LEAK` | 同长度换内容就变了 ⇒ ref 看见了 txt | **🔴 停**,发给作者 |
| `❌ FAIL_STEP` | cond 支路都不再步不变(第一轮没出现) | **🔴 停**,发给作者 |
| `⚠️ INCONCLUSIVE` | A 复算都重现不了缓存 ⇒ 探针机制有问题 | **🔴 停**,发给作者 |

顺带多记了 mean\|Δ\| 和不等元素占比(第一轮只有 max)。这些是**记录不是门禁**,
连同噪声速度相对 L2 一起写进报告。

**上一轮的主批处置**:你在门禁出结论前先排队、FAIL 后 kill 掉,没浪费资源,做法可以。
这轮同样可以先排队,但 `PASS` 之外的任何结论都要 kill。
