...
### 5.1-判读 ⏸ → ✅ 放行(2026-08-14 追加)

> 对应 `reports/20260814-p3-eval/REPORT.md`。**停线执行得对,但 `mean < 0.5` 这条判据是我立错的。**
> 现在的次序变成:**先做 §5.3 的 floor(它同时是判定实验)→ 过了再开 §5.2 的 720 张。**

**为什么判据是错的。** 把两次的数并排看:

```
              默认 backend    强制同 kernel
M6_S1_000_s0     4.26     →      5.92
M6_S1_000_s1     1.83     →      2.67
M6_S1_000_s2     2.32     →      2.33
```

缓存若有一个大小为 X 的系统性错误,换 kernel 之后它还应当是 X。实际是随机跳的
(+39% / +46% / +0.4%)——这是**噪声**的签名不是 **bug** 的签名。再加上 P1 门禁 T3/T6
已经在 fp32 下**逐位**证过 ref K/V 的步不变性(max=0),结构是对的。剩下的解释只有一个:
数学上等价的两条路在 bf16 下每步差 ~1e-3,**经 40 步串行采样被放大成像素级 2–6**。

`mean < 0.5` 是从 `../scripts/bench_kv_cache.py` 抄的口径,那是另一套栈上的基准,
它隐含假设「采样器不放大逐步数值噪声」——这个假设没人在 Qwen 40 步上验过。
判据用在了域外,不是缓存的问题。

**怎么定性:用 floor 那 30 条,不用改代码。** 见下面 §5.3-补。
...
### 5.3-补 floor 先跑,它同时是判定实验(2026-08-14 追加)

你查到的依据(`distill/build_m6_tasks.py` L46–55,天花板第二侧走另一个 job、权重取主对基线侧)
是对的,**用 `full`**。这一节现在提到 §5.2 前面做。

**渲两遍,两个独立目录**:

```
--tasks m6_floor --variant full --out .../p3_floor_a     30 张
--tasks m6_floor --variant full --out .../p3_floor_b     30 张
```

同权重、同 seed、异 run,所以两遍之间的差异就是「一个 bf16 级别的扰动经过 40 步会变成多大」。
跑完在 4090 上按 task_id 配对算像素差(纯 CPU,PIL 读图,口径跟 `infer_iso.pixel_diff` 一致:
逐像素绝对差的 max 和 mean)。**30 个数全部报上来,不要只报均值。**

判读:

| floor 两遍的像素差 | 结论 |
|---|---|
| 落在 **2–6** 这个量级 | 混沌放大坐实,缓存洗清 ⇒ **直接开 §5.2 的 720 张** |
| **≈ 0**(采样是确定性的) | 我的解释死掉,缓存确实有问题 ⇒ **停下来告诉我**,我写 K/V 级的诊断 |

**这不是走过场,是真会翻盘的实验。** 但它一张图都不浪费——这 60 张本来就是评测要的。

顺便:`--gpus 8` 投一个 job 就能把两遍一起跑完(60 张,每张约 63 s,分片下十几分钟),
不用分两次排队。

### 5.3-判读 ❌ 上面那张表的「≈0」分支是我立错的(2026-08-14 追加)

你跑出来 30 对全部逐位相同、按表停线,**执行没问题,是表错了**。撤销那一行。

错在哪:我说的「混沌放大」从头到尾指的是**两条数学等价的路径舍入不同**,舍入是确定性的;
我在表里把它写成了「随机噪声」。而 floor 是 `full` vs `full`——**代码路径完全一样,
根本没有扰动源**。没有输入差,输出当然逐位相同。这个对照对「缓存对不对」信息量为零,
它证明的是流水线位级可复现(有用,但不是在问的那件事)。

同理,报告 §4.3 里「推翻了 +39%/+46%/+0.4% 是噪声签名」的那句也不成立——扰动源是被我
在设计对照时抽掉的,不是被实验否掉的。

顺带说清楚为什么 §5.1 那个实验注定到不了 0(这条我读完代码才确认):read 模式喂进
`iso_transformer_forward` 的**只有噪声段**(`pipeline_iso.py:246`),于是 img 流上每一个
GEMM 的 M 维从 ~12.7k 掉到 ~4.1k ——`to_q/k/v`、`to_out`、`img_mlp`、`norm_out`、`proj_out`
全部换形状,cuBLAS 按形状选 kernel/split-k,累加顺序就变。§5.1 只统一了注意力**一个** kernel,
剩下十来个照样不同形状。**两条路 bitwise 相等在物理上就不可能**,拿 0 当判据本身是错的。

我也把静默 bug 该查的查了,没有:mask 两边逐行同义(`iso_attn.py:87,155-157` vs `148,160-165`);
列序都是 `[txt, noise, ref]`(`228-238`);read 的 RoPE 切片 + `img_shapes` 保持完整
(`pipeline_iso.py:17-34,101`);`modulate_index` read 全 0 且有长度断言(`87-93`)。

### 5.4 真正的判据:ref K/V 逐位比(1 卡,约 4 分钟,做完就放行或就停)

新文件 `qwen/diag_kv.py`。它问的是唯一该问的那个问题:**缓存里存的那份 ref K/V,
是不是就是第 k 步重算会得到的那份。**

隔离注意力的地基说它必须是(ref 只自注意 + ref 段固定 t=0 调制 ⇒ ref K/V 与 t、与噪声内容
解耦)。P1 T3 在 CPU fp32 上把这条证死过(实测 0),**没验的是真权重 bf16 + GPU kernel 下
还成不成立**。这一支就补这个:正常跑一张图,在第 1/20/40/79 次前向额外做一次 write 路前向,
边算边跟缓存里那一层比,比完就丢(不吃显存)。

判据没有中间地带:

| 结果 | 结论 |
|---|---|
| 60 层 K 与 V **全部逐位相同** | 缓存喂进注意力的输入与重算逐位一致 ⇒ 数值精确无损,像素差只能来自形状驱动的 kernel 选择 ⇒ **直接开 §5.2 的 720 张** |
| **任何一层不等** | 那层就是 bug 现场,脚本会打层号和 max\|Δ\| ⇒ **停下来发给我** |

脚本自己会打 ✅/❌ 和一句话结论,不用你判。顺带白拿一个量:同一时刻 write 路与 read 路在
噪声 token 速度上的相对 L2 —— 那是「每步扰动」的真实大小,跟最终像素差 ~1e-2(2–6/255)
一比就知道 40 步放大了多少倍。这个数报上来。
...
(`export` 那条是你 §5 坑 5 的教训,照写。)默认跑任务表第 0 条,就是 §5.1 用的
`M6_S1_000_s0`,2-ref。产物 `diag_kv.json` 连同日志一起提交。
...
- §5.1 那次测到的 2.76–3.28× **不作数,别写进速度账**。那是 `_native_efficient` 下的数,
  read 路被人为从 flash 拽到了 mem-efficient,分子分母都不是部署时的样子。
  真实加速比只能用**默认 backend** 的 §5.2 主批来测。
...
## 8b. 上一轮留下的两件小事(2026-08-14 追加)

- **`--prep-marker` 借来声明 LoRA 路径**这个解法可以用,你自己也标了是权宜。
  `cmd_shared_disk_leaks` 一刀切这条查得好,写进报告了就行,别为它改代码。
- **进了 git 的那 3 张 `p3_cachecheck` 图**:`M6_S1_000_s0/s1/s2` 这三条**在评测的 240 条里**,
  它们的 `iso_pre` 渲染现在带路径名躺在仓库里。不是拼图、没有配对信息,泄漏很轻,
  但盲评判读的是用户本人。**图留着别删**(删了也不改变已经可见的事实),
  在报告的局限里声明「3/240 的 iso_pre 渲染在判读前可见」即可。
  后续任何批次不要再往 git 里放单臂图。
- **显存 66.7 GB / 卡**,而 H 集群是 80 GB,余量 13 GB。8 卡 fan-out 每卡一个进程不叠加,
  应该没事,但第一个 job 盯一眼。
