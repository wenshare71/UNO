...
**注意:数据集没有 ground-truth 目标图**,benchmark 靠 VLM-as-judge(`judge.py`,
Gemini/GPT/Qwen3-VL)打 5 维分,不是比像素——定量只能靠 judge 或身份相似度,算不了 GT 距离。
...
同时把主变体(ours_kv)结果写回各任务目录 `<num>_generated.jpg`,可直接跑
MultiBanana 的 `judge.py` 打分。

## 参数备忘

- `--ref_size 512`:ckpt7000 对照实验里 512 的多主体保留明显好于 320(默认已是 512)
