...
> 数据口径：全文只用**旧口径**与**加速比**两类数字，其余统计量一律不进正文。
...
两轮做完，账是：**2.26× 加速，四张里有三张多，人眼看不出和原模型的差别。**
...
## 2. 先量一次尺子（否则后面所有数字都没意义）

全部结果来自**单标注者的成对盲评**：左右两张图随机摆放，标注者只回答「哪张更好，或打平」，看不到变体名。
...
1. **它不是概率**，可以超过 1。
2. **它被平局稀释。** 平局越多，比值越被拉向 1。
3. 所以**每一批都必须在同一批里混入「两侧是同一份权重」的锚点**，量出这把尺子在这一场里的**满分刻度**。实测满分是 **1.000–1.017**——不是靠猜是 1.0。

⇒ **任何一个旧口径都必须连平局率一起读，并且只跟自己批内的满分刻度比。**跨批次的读数不能当同一把尺子并排引用（尺子会漂：跨天自洽率 55.0%）。

---

## 3. 第一轮：在 FLUX 底座上，用 SFT 把能力训回来
...
### 3.3 实验 2 —— 重训之后，能追平官方吗？

**方法**：第 3.1 节那两段 SFT 训完，与官方权重成对盲评 227 条。
...
### 3.4 实验 3–5 —— 把三件事拆开，每次只动一个变量

做法是造一条链，链上每一步**只差一个变量**：

| 只动这一个 | 具体做法 | 旧口径 | 批内满分刻度 | 平局率 |
|---|---|---|---|---|
| 训练数据配方 | 官方权重为起点，**不开隔离**，训同样 4000 步 | 0.901 | 1.017 | 59.4% |
| **隔离结构** | 与上一行**只差一个开关** | **0.842**<br>（2-ref **0.716** / 1-ref 1.122） | **1.000** | 51.6% |
| 底座 | 只把起点从官方权重换成自己复刻的 | 1.007 | 1.000 | 53.6% |
...
- **代价存在，方向确定，几乎全在多主体**：2 参考图旧口径 0.716（批内满分是 1.000），1 参考图 1.122——**在单参考图上测不到代价**。
- **掉的不是主体身份**：46/51 与全注意力的 45/51 分不开。丢的是构图、主体间关系、画面质量，这一批分不开是哪一个。
- **底座不承重**：换成自己复刻的底座，旧口径 1.007，而该批满分刻度正好 1.000——**点估计贴在天花板上**。
- **训练数据配方本身就有代价**：0.901 对 1.017，两者可分辨。这一条是 SFT 路线自带的——因为它同时在教「生成好图」，而不只是「在新结构下复现旧行为」。
- 买回来的是 **1.672×**。

---

## 4. 第二轮：换一种训练目标——不造数据，直接对齐 teacher
...
**为什么**：图全出完才发现缓存是坏的，那批图全废。

**方法**：三支探针，把每一层缓存的 K/V 与首次写入的值逐位比对——
...
**读法**：A 证明探针本身成立；C 证明参考图段对文本**内容**完全免疫——隔离掩码确实生效；B 只有长度变了却几乎全层不同。
...
| | 旧口径 | 平局率 |
|---|---|---|
| **隔离学生 vs 官方全注意力** | **0.946** | 81.7% |
| ├ 2 参考图 | 0.914 | 75.2% |
| └ 1 参考图 | 1.014 | 96.0% |
| **满分刻度**（两侧逐位相同的图，30 对） | **1.000** | 100.0% |

**读懂 0.946 的唯一方式，是同一双眼睛、同一场次的三个刻度**：

| 比的是什么 | 平局率 |
|---|---|
| 两张**逐位相同**的图 | **100%**（30/30） |
| **隔离学生 vs 官方** | **81.7%** |
| **不重训直接开隔离** vs 官方 | **< 2%** |

最后一行是关键的对照：真有差别的时候，这把尺子会塌到 2% 以下——**所以 81.7% 不是尺子瞎了**。不重训直接开隔离的那一支，生成图里根本看不到参考图的特征，这与第一轮实验 1 的 0/51 是同一个现象，在新底座上原样复现。

> ⚠️ 最后一行是**目视判定，非盲评**，与前两行不是同一个数据来源。它作为量程的下端锚点使用，不作为读数。
...
| | 训练方法 | 旧口径（总体） | 2 参考图 | 平局率 | 加速 |
|---|---|---|---|---|---|
| 第一轮 | 两段图像 SFT（含 40% teacher 造图） | 0.819 | 0.770 | 41.9% | 1.672× |
| **第二轮** | **速度匹配自蒸馏，不造数据** | **0.946** | **0.914** | **81.7%** | **2.26×** |
...
**判据**：第二轮的预登记达标判据**判据不适用**——它要求非平局样本 ≥ 94，实测 44。平局率 81.7% 意味着要凑够得评 513 对，而整个评测集只有 320 条任务。**这是关于尺子长度的陈述，不是关于模型的**；按预登记，样本不足时不许事后追加样本。
...
flowchart TD
    subgraph R1["第一轮 · FLUX.1-dev 冻结主干 · LoRA rank 512 · 图像 SFT"]
        ...
        ARMA["全注意力对照腿<br/>4000 步 · 旧口径 0.901"]
        ARMB["隔离腿<br/>4000 步 · 旧口径 0.842"]
        POST["自训底座 + 隔离<br/>4000 步 · 旧口径 0.819"]
    ...
    subgraph R2["第二轮 · Qwen-Image-Edit-2511 冻结主干 · LoRA rank 64 · 速度匹配自蒸馏"]
        ...
        QPOST["隔离学生<br/>2000 步 · 旧口径 0.946 · 2.26×"]
