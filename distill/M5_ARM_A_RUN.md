...
**照抄 `M5_PROBE_RUN.md` 的写法,单卡不分 shard。** WHY:8 shard 能把 40 min 压到
~11 min,但要多出 8 个进程、8 份日志和一次合并——为省 29 min 在**产出 P1 读数的
那一批**上引入新失败面不划算。P-probe 单卡 25.8 min / 0 失败这条路已经趟过。
...
**方向正确**(隔离在训练里更贵,臂 A 更快是预期内的;**跑出 5.6 附近才是警报**)。
