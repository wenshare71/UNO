...
>
> **⬜ 由此打开的一个问题(待查,10 秒)**:§11.10 把 `run_floor` 唯一那条非平局
> (`S1_039_s2`)记成"标注者约 1.7% 的误判底噪"。**若 `S1_039_s2` 正是尾巴上的条目,
> 这个归因就是错的**——那不是误判,是那张图真的重生成成了另一个样子,
> 两种读法对"尺子的误判底噪"给出 1.7% vs 0% 的相反估计。查 `S1_039_s2` 的
> mean|Δ| 即可判定。**对边 ① 与臂 B 的设计均无影响**:臂 B 终批的 `run_floor`
> 是同会话间隔的新对照(§11.11 本地加固条),这条尾巴会被它当场量到,不需要外推。
> ```bash
> python - <<'PY'
