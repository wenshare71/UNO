...
P1 ◐  实测下载速率 ✅ ──► 回填 §4 ✅ ──► 补数据(16.4 h)──► build_stage1_official --strict
P2    两腿 stage-1,串行:student 86 h + baseline 74 h
P3    两腿蒸馏,各 4000 步:合计 ~12 h
P4    扩任务池 192→320 ──► 两腿生图 ──► build_pairs.py m6 ──► 盲评(~25 min)──► report
P5    报告
```

GPU 总账 **≈ 172 h ≈ 7.2 天**(8 卡独占,串行)。数据准备 16.4 h 不占 GPU,可与其它事并行。
