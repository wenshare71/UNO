...
`distill/REMOTE_AGENT_HANDBOOK.md` §2.0 写明「**代码不由你写**……你不写 `.py`,也不改 `.py`」,
并声明该条优先级高于手册其余所有条款。

**用户于 2026-07-30 显式解除该限制,范围仅限本文档列出的两个新文件:**

- `distill/build_eval_json.py`(新建)
- `distill/eval_multiref.py`(新建)

**边界没有放宽的部分,继续按手册执行**:

- 除上述两个文件外,**任何既有 `.py` / `.sh` 一律不改**。需要改 → 按手册 §3 出诊断包上报;
- 手册 §0 铁律不变:**你可以修"让代码跑起来"的问题,不可以修"改变实验含义"的问题**。
  本文档 §2 是"实验含义"的完整清单,清单内的任何一项与你的判断冲突时,**停下上报,不要自己定夺**;
- 绿灯 G1–G6 照旧。

**为什么解除**:这两个脚本的实验语义已经在本文档里被固定到"没有自由度"的程度
(组合枚举规则、seed 公式、视角选取规则、变体配置全部写死),剩下的只是把规格翻译成代码。
语义泄漏的风险因此从"代码里"转移到了"本文档里",而本文档你我都能核对。
